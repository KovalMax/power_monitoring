...
class EventService:
    ...
    def create_new_event(self, event: PowerStateUpdateEvent) -> None:
        model = EventModel(device_id=event.device_id, power_state=event.power_state, network_level=event.network_level,
                           battery_level=event.battery_level, created_at=event.fired_at, updated_at=event.fired_at)
